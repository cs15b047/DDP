...
opt = [torch.optim.Adam(models[i].parameters(), lr = lr, weight_decay = 0) for i in range(num_models)] ############# Weight decay --> hyperparam #################

models = [nn.DataParallel(mod, device_ids = [0, 1]) for mod in models]

for i in range(num_models):
	models[i].cuda()

#Load model from file
load = int(input("Load model: ")) # take input
if(load == 1):
	ep = int(input("Epoch to load: "))
	for i in range(num_models):
...
for epoch in range(n_epoch):
	...
	for idx, (images, labels, ind) in enumerate(tqdm(train_loader)):
		...
		# Shorten an epoch to 100k examples
		if(idx >= 800):
			break
	...
	if(epoch >= strt_ep):
		...
		##Normalization of weights --DDP Review comment
